...
class LibiglConan(ConanFile):
    ...
    git_hash = "66b3ef2253e765d0ce0db74cec91bd706e5ba176"
    license = "MPL-2.0"
    ...
    def source(self):
        self.run("git clone --single-branch https://github.com/libigl/libigl.git {}".format(self._source_subfolder))
        self.run("cd " + self._source_subfolder + " && git checkout " + self.git_hash)

    def _configure_cmake(self):
        if not self._cmake:
            self._cmake = CMake(self, parallel=False)
            self._cmake.definitions["CMAKE_POLICY_DEFAULT_CMP0048"] = "NEW"
    ...
    def package_info(self):
        ...
        self.cpp_info.components["igl_common"].includedirs = [
            "{}/include".format(self._source_subfolder)
        ]